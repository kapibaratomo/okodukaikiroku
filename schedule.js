...
document.getElementById('prevWeek').addEventListener('click', () => {
    ...
    currentWeekStart = new Date(currentWeekStart); // 新しいDateオブジェクト
    buildTable();
});
...
async function loadDataFromGitHub() {
    ...
    try {
        ...
        localStorage.setItem('schedule_data_backup', JSON.stringify({ schedule: scheduleData }));
    } catch (e) {
...
function loadDataFromLocalFallback() {
    const saved = localStorage.getItem('schedule_data_backup');
    if (saved) {
        try {
